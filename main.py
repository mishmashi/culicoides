...
if st.session_state.index < len(questions):
    if st.session_state.prior:
        if st.button("Skip to ranking", key="skip", width='stretch'):
    ...
    if q_b != "" and q_b != None:
        ...
        if os.path.exists(imgstrunique):
            ...
            if os.path.exists(imgstry):
                tr.image(imgstry, width='stretch')
            if os.path.exists(imgstrn):
                fal.image(imgstrn, width='stretch')
...
if st.session_state.index > 0:
    if bn1.button("Previous question",key="prev_spec", width='stretch')  and st.session_state.answered:
